...
public class AlphaBetaPruning implements TreeEvaluator {
    ...
    public GameNode evaluateTree(GameTree gameTree, int depth) {
        ...
        GameNode bestMove = null; //specific to this first iteration
        int alpha = Integer.MIN_VALUE;
        int beta = Integer.MAX_VALUE;
        GameNode root = gameTree.getRoot();
        List<GameNode> children = root.getChildren(); //get or calculate children
        if (root.getPosition().getWhiteNextMove()) {
...
/*     private int alphaBetaPruningMiniMax(GameNode node, int depth, int alpha, int beta) {
     ...
     * @return
     */
    private int alphaBetaMinimize(GameNode parent, int depth, int alpha, int beta) {
        //assign static evaluation to leaves
        if (depth == 0) { //FIXME nodes may be leaves before depth = 0 e.g. check mate boards have no children
     ...
     * @return
     */
    private int alphaBetaMaximize(GameNode parent, int depth, int alpha, int beta) {
        //assign static evaluation to leaves
        if (depth == 0) { //FIXME nodes may be leaves before depth = 0 e.g. check mate boards have no children
            return PositionEvaluator.evaluatePosition(parent.getPosition());
        }
