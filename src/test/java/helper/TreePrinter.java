...
public class TreePrinter {
    ...
    */
    public static void alignAndPrintTree(Tree<Node<Position>> tree, int depth) throws ComputeChildrenException {
        alignNode(tree.getRoot(), 0, depth);
        updateAlignments(tree.getRoot(), 0, depth);
        printTree(tree, depth);
    }

    public static void alignAndPrintTree(Node<Position> node, int depth) throws ComputeChildrenException {
        alignNode(node, 0, depth);
        updateAlignments(node, 0, depth);
     ...
     */
    private static void updateAlignments(Node<Position>  node, int currentDepth, int treeMaxDepth) throws ComputeChildrenException {
        ...
        for (Node<Position>  child : children) {
            updateAlignments((Node<Position> ) child, currentDepth + 1, treeMaxDepth);
        }
    }

    private static String gamenodeToString(GameNode node) {
        if (node.getRepresentedMove() == null) {
            return "null";
        } else {
            return node.getRepresentedMove().toStringAlgebraic();
        }
    }
     ...
     */
    private static int alignNode(Node<Position> node, int currentDepth, int treeMaxDepth) throws ComputeChildrenException {
        if (!node.hasChildren()) {
            //align representation
            String string = gamenodeToString((GameNode)node);
            ...
            if (currentDepth < treeMaxDepth) {
                ...
                node.insertChild((Node<Position>)new AlignmentNode());
            } else {
        ...
        for (int i = 0; i < children.size(); i++) {
            child = (Node<Position> ) children.get(i);
            childrenWidth += alignNode((Node<Position> ) child, currentDepth + 1, treeMaxDepth);
        }

        Node<Position>  firstChild = ((Node<Position> ) children.get(0));
        injectLeft(firstChild.getAlignedRepresentation(), CHILDREN_START_CHARACTER);

        Node<Position>  lastChild = ((Node<Position> ) children.get(children.size() - 1));
        injectRight(lastChild.getAlignedRepresentation(), CHILDREN_END_CHARACTER);

        node.setAlignedRepresentation(alignGuaranteeWhiteSpace(gamenodeToString((GameNode)node), childrenWidth));
        return node.getAlignedRepresentation().length();
    }
     ...
     */
    private static void printTree(Tree<Node<Position> > tree, int treeDepth) throws ComputeChildrenException {
        //print each layer
    ...
    private static void printTree(Node<Position>  root, int treeDepth) throws ComputeChildrenException {
        //print each layer
        for (int i = 0; i < treeDepth + 1; i++) {
            printNodes(root, i, 0, treeDepth);
            System.out.print("\n\n"); //newline
        }
    }
     ...
     * @throws ComputeChildrenException
     */
    private static void printNodes(Node<Position>  node, int layerDepth, int currentDepth, int treeMaxDepth)
            throws ComputeChildrenException {
        ...
        List<? extends Node<Position> > children = node.queryChildren();
        for (Node<Position> child : children) {
            printNodes((Node<Position>) child, layerDepth, currentDepth + 1, treeMaxDepth);
        }
    }
}
